...
服务器除了活动工具，还有一条双线迁移账本。账本自己不访问佳明或高驰。它配合 `dsh-plugin-garmin-connect`：佳明插件负责下载 FIT，本服务器负责上传到高驰。两个方向的进度分开记在 `~/.coros-additional-mcp/migration.json`。
...
- `migration_status` / `migration_enqueue` / `migration_next` / `migration_record` — 排队、每次只领一个到期步骤、写回结果。成功后停 45–85 秒。失败从 30 秒起指数退避，上限 15 分钟。`migration_next` 要求等待时，不要再传下一个文件。
- `migration_locate_garmin_fit` — 佳明插件下载完成后，在 `GARMIN_FIT_DOWNLOAD_DIR` 里找到 `{activityId}.fit`。
